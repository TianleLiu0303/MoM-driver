...
class AgentInput:
    ...
    def from_scene_dict_list(
        ...
        num_of_real_history = len(scene_dict_list) - num_future_frames
        global_ego_poses = []
        for frame_idx in range(num_of_real_history):
...
class Scene:
    ...
    def get_future_trajectory(self, num_trajectory_frames: Optional[int] = None) -> Trajectory:
        ...
        start_frame_idx = len(self.frames) - self.scene_metadata.num_future_frames - 1
    ...
    def get_history_trajectory(self, num_trajectory_frames: Optional[int] = None) -> Trajectory:
        ...
        global_ego_poses = []
        num_of_real_history = len(self.frames) - self.scene_metadata.num_future_frames
        assert num_of_real_history > 0, f"Ensure there are enough frames to extract history, got num_of_real_history={num_of_real_history}, self.frames={len(self.frames)}, scene_metadata={self.scene_metadata}"
        for frame_idx in range(0, num_of_real_history):
            global_ego_poses.append(self.frames[frame_idx].ego_status.ego_pose)
    ...
    def from_scene_dict_list(
        ...
        """
        assert len(scene_dict_list) >= 0, "Scene list is empty!"
        now_frame_index = -num_future_frames - 1
        scene_metadata = SceneMetadata(
            log_name=scene_dict_list[now_frame_index]["log_name"],
            scene_token=scene_dict_list[now_frame_index]["scene_token"],
            map_name=scene_dict_list[now_frame_index]["map_location"],
            initial_token=scene_dict_list[now_frame_index]["token"],
...
class SceneFilter:
    ...
    def __post_init__(self):
        ...
        print("frame interval:", self.frame_interval)
