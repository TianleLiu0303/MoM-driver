...
from navsim.planning.training.dataset import pad_sequence, pad_status


class RWKVFeatureBuilder(AbstractFeatureBuilder):
    """Input feature builder for RWKV."""
    ...
    # for only lastest frame
    def compute_features(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        """Inherited, see superclass."""
        features = {}

        actual_seq_len = len(agent_input.cameras)
        camera_feature = [self._get_camera_feature(agent_input, index=i) for i in range(actual_seq_len)]
        lidar_feature = [self._get_lidar_feature(agent_input, index=i) for i in range(actual_seq_len)]

        features["camera_feature"] = torch.stack(camera_feature, dim=0)
        features["lidar_feature"] = torch.stack(lidar_feature, dim=0)
        ...
        features["camera_feature"], padding_num = pad_sequence(features["camera_feature"], self._config.cam_seq_len)
        assert padding_num == self._config.cam_seq_len - actual_seq_len
        features["lidar_feature"], padding_num = pad_sequence(features["lidar_feature"], self._config.lidar_seq_len)
        assert padding_num == self._config.lidar_seq_len - actual_seq_len
        return features
    ...
    #         torch.concatenate(
    #             [
    #                 torch.tensor(agent_input.ego_statuses[i].driving_command, dtype=torch.float32),
    #                 torch.tensor(agent_input.ego_statuses[i].ego_velocity, dtype=torch.float32),
    #                 torch.tensor(agent_input.ego_statuses[i].ego_acceleration, dtype=torch.float32),
    #             ]
    ...
    #     features["camera_feature"] = torch.stack(camera_feature, dim=0)
    #     features["lidar_feature"] = torch.stack(lidar_feature, dim=0)
    #     features["status_feature"] = torch.stack(status_feature, dim=0)
    ...
    #     features["lidar_feature"], padding_num = pad_sequence(features["lidar_feature"], self._config.lidar_seq_len)
    #     assert padding_num == self._config.lidar_seq_len - actual_seq_len
    #     features["status_feature"], padding_num = pad_status(features["status_feature"], self._config.cam_seq_len)
    #     assert padding_num == self._config.cam_seq_len - actual_seq_len
    #     return features

    def _get_camera_feature(self, agent_input: AgentInput, index=-1) -> torch.Tensor:
        """
        ...
        cameras = agent_input.cameras[index]
    ...
    def _get_lidar_feature(self, agent_input: AgentInput, index=-1) -> torch.Tensor:
        """
        ...
        lidar_pc = agent_input.lidars[index].lidar_pc[LidarIndex.POSITION].T
...
class RWKVTargetBuilder(AbstractTargetBuilder):
    """Output target builder for RWKV."""
    ...
    def compute_targets(self, scene: Scene) -> Dict[str, torch.Tensor]:
        ...
        frame_idx = len(scene.frames) - scene.scene_metadata.num_future_frames - 1
    ...
    def _compute_agent_targets(self, annotations: Annotations) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        def _xy_in_lidar(x: float, y: float, config: RWKVConfig) -> bool:
