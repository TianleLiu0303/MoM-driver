...
class RWKVBackbone(nn.Module):
    ...
    def forward(self, image, lidar, past_features=[None, None, None, None], use_cache=False, frame=None):
        """
        ...
        for i in range(4):
            image_features = self.forward_layer_block(image_layers, self.image_encoder.return_layers, image_features)
            lidar_features = self.forward_layer_block(lidar_layers, self.lidar_encoder.return_layers, lidar_features)
            image_features, lidar_features, past_features[i] = self.fuse_features(image_features, lidar_features, i, past_features[i], use_cache, frame)
        ...
        return features, fused_features, image_feature_grid, past_features
    ...
    def fuse_features(self, image_features, lidar_features, layer_idx, past_feature=None, use_cache=False, frame=None):
        """
        ...
        print(f"Before RWKV - image: {image_embd_layer.shape}, lidar: {lidar_embd_layer.shape}, frame: {frame}")  

        image_features_layer, lidar_features_layer, past_feature = self.rwkvs[layer_idx](image_embd_layer, lidar_embd_layer, past_feature, use_cache, frame)
        ...
        return image_features, lidar_features, past_feature


class GPT_RWKV(nn.Module):
    """The full GPT language backbone, with a context size of block_size."""

    def __init__(self, n_embd, num_heads, config: RWKVConfig, lidar_time_frames):
        super().__init__()
    ...
    def forward(
            self, image_tensor, lidar_tensor,
        ...
        x = self.drop(self.pos_emb + token_embeddings) # (B * seq_len, T, C), T = img_ver * img_horz + lidar_ver * lidar_horz
        x = x.view(batch_size, -1, self.n_embd)
        
        v_first = None
        attention_mask = None
        if frame is not None:
            feature_length = self.img_feature_size + self.lidar_feature_size
        ...
        return image_tensor_out, lidar_tensor_out, past_key_values
