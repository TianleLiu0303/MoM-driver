...
from navsim.agents.rwkv_block.rwkv7_block_fla import RWKV7_DoubleSelfAttnBlock, RWKV7_CustomCrossAttnBlock
...
class RWKVModel(nn.Module):
    ...
    def forward(
        ...
        bev_feature_upscale, bev_feature, _, past_features = self._backbone(camera_feature, lidar_feature, past_features, use_cache)
...
class RWKV_DiffusionDeCoderLayer(nn.Module):
    def __init__(self, idx, num_poses, config: RWKVConfig) -> None:
        ...
        self.cross_agent_attention = RWKV7_CustomCrossAttnBlock(
            hidden_size=config.rwkv_d_model,
            norm_first=config.rwkv_d_norm_first,
            num_heads=config.rwkv_d_num_head,
            decay_low_rank_dim=config.decay_low_rank_dim,
            ...
            v_low_rank_dim=config.v_low_rank_dim,
            fuse_norm=config.fuse_norm,
            n_layer=config.diffusion_decoder_layer_nums,
            layer_idx=idx
        )
        
        self.cross_ego_attention = RWKV7_CustomCrossAttnBlock(
            hidden_size=config.rwkv_d_model,
            norm_first=config.rwkv_d_norm_first,
            num_heads=config.rwkv_d_num_head,
            decay_low_rank_dim=config.decay_low_rank_dim,
        ...
        self.time_modulation = ModulationLayer(config.rwkv_d_model, 256)
    ...
    def forward(
            ...
            ego_query, time_embed, qv1_first=None, qv2_first=None, 
            agent_v_first=None, ego_v_first=None, global_img=None
        ):
        # spatial cross attention
        traj_feature = self.cross_bev_attention(traj_feature, noisy_traj_points, bev_feature, bev_spatial_shape)

        # agent cross attention
        traj_feature, qv1_first, agent_v_first = self.cross_agent_attention(traj_feature, agents_query, qv1_first, agent_v_first)
        traj_feature = self.norm1(traj_feature)

        # ego cross attention
        traj_feature, qv2_first, ego_v_first = self.cross_ego_attention(traj_feature, ego_query, qv2_first, ego_v_first)
        ...
        return poses_reg, poses_cls, qv1_first, qv2_first, agent_v_first, ego_v_first
...
class RWKV_DiffusionDecoder(nn.Module):
    ...
    def forward(
        ...
        for layer in self.layers:
            poses_reg, poses_cls, qv1_first, qv2_first, agent_v_first, ego_v_first = layer(
                traj_feature, traj_points, bev_feature, 
                bev_spatial_shape, agents_query, ego_query, 
                time_embed, qv1_first, qv2_first, 
                agent_v_first, ego_v_first, global_img
            )
