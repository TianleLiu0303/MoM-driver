...
class RWKVModel(nn.Module):
    ...
    def __init__(self, config: RWKVConfig):
        ...
        tf_decoder_layer = nn.TransformerDecoderLayer(
            ...
            nhead=config.rwkv_d_num_head,
            dim_feedforward=config.rwkv_d_ffn,
            dropout=config.tf_dropout,
            batch_first=True,
        )
        self._tf_decoder = nn.TransformerDecoder(tf_decoder_layer, config.rwkv_d_num_layers)
    ...
    def forward(
        ...
        agents_query = self._tf_decoder(agents_query, keyval)
...
class RWKV_DiffusionDeCoderLayer(nn.Module):
    def __init__(self, idx, num_poses, config: RWKVConfig) -> None:
        ...
        self.agent_dropout = nn.Dropout(0.1)
        
        self.cross_bev_attention = nn.MultiheadAttention(
            config.rwkv_d_model,
            config.rwkv_d_num_head,
            dropout=config.tf_dropout,
            batch_first=config.rwkv_d_norm_first,
        )
    ...
    def forward(
        ...
        traj_feature = traj_feature + self.bev_dropout(self.cross_bev_attention(traj_feature, bev_feature, bev_feature)[0])
        traj_feature = self.norm1(traj_feature)

        # bev cross attention
        traj_feature = traj_feature + self.agent_dropout(self.cross_agent_attention(traj_feature, agents_query, agents_query)[0])
