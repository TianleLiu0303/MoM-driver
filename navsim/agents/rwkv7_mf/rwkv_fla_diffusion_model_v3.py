...
class RWKVModel(nn.Module):
    ...
    def forward(
        ...
        print("camera_feature_shape:",camera_feature.shape)
        ...
        print("bev_feature_upscale:", bev_feature_upscale.shape)
        ...
        bev_feature = bev_feature.permute(0, 2, 1)  # [bs*10,64,256]
        expanded_bs, T, C = bev_feature.shape
        print("expanded_bs:", expanded_bs, "T:", T, "C:", C )

        # embed the keyval
        keyval_bev = bev_feature + self._bev_feature_embedding.weight[None, ...]
        keyval_bev = keyval_bev.contiguous().view(bs, -1, C)  # [bs,10*64,256]
        keyval_bev = self.fusion_drop(keyval_bev)

        print("keyval_bev.shape:", keyval_bev.shape)
        ...
        print("keyval_status.shape:", keyval_status.shape)
        ...
        print("agents_query.shape:", agents_query.shape)
        ...
        cross_bev_feature = cross_bev_feature.flatten(-2, -1).permute(0, 2, 1).contiguous().view(bs, -1, C)  # [bs,10*T, 256]
        print("cross_bev_feature.shape:", cross_bev_feature.shape) # (3,2560,2560)
...
class TrajectoryHead(nn.Module):
    ...
    def forward_train(self, agents_query, bev_feature, targets=None, global_img=None) -> Dict[str, torch.Tensor]:
        ...
        print("noisy_traj_points.shape:", noisy_traj_points.shape)
        ...
        print("traj_feature.shape:", traj_feature.shape)
...
if  __name__ == "__main__":
    ...
    B=3
    seq_len = 10
    camera_data = torch.rand((B,seq_len, 3, 256, 1024)).to(device)
    lidar_data = torch.rand((B, seq_len, 1, 256, 256)).to(device)
    status_data = torch.rand((B, 8)).to(device)

    total_features = {
        "camera_feature": camera_data,
        "lidar_feature": lidar_data,
        "status_feature": status_data
    }
  
    print(total_features["camera_feature"].shape)
