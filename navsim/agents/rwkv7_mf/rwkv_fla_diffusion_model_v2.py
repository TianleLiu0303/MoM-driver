...
class DiffMotionPlanningRefinementModule(nn.Module):
    ...
    def forward(self, traj_feature):
        ...
        plan_score = self.plan_score_branch(traj_feature)
        plan_score2 = None
        pred_agent_col = self.agent_col_branch(traj_feature).reshape(bs, ego_fut_mode, self.ego_fut_ts, -1, 2, 9) # (4, 100, 8, 5, 2, 9)
        pred_area = self.area_branch(traj_feature)

        if self.use_double_scorer:
            plan_score2 = self.plan_score_branch2(traj_feature)
