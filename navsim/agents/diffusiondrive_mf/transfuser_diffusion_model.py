...
from navsim.agents.diffusiondrive_mf.transfuser_backbone import TransfuserBackbone
from navsim.agents.diffusiondrive_mf.transfuser_features import BoundingBox2DIndex
...
class TransfuserModel(nn.Module):
    ...
    def __init__(self, config: TransfuserConfig):
        ...
        self._keyval_embedding = nn.Embedding(config.lidar_seq_len * 8**2 + 1, config.tf_d_model)  # seq_len * 8x8 feature grid + trajectory
    ...
    def forward(
        ...
        camera_feature = camera_feature.view(bs * seq_len, C, H, W)  # (4 * B, _, _, _)

        bs, seq_len, C, H, W = lidar_feature.shape
        lidar_feature = lidar_feature.view(bs * seq_len, C, H, W)  # (4 * B, _, _, _)

        if use_cache:
            bev_feature_upscale, bev_feature, _ = self._backbone(camera_feature, lidar_feature, True)
        else:
            bev_feature_upscale, bev_feature, _ = self._backbone(camera_feature, lidar_feature, False)
        ...
        # get the query
        query = self._query_embedding.weight[None, ...].repeat(bs, 1, 1)
...
class CustomTransformerDecoderLayer(nn.Module):
    def __init__(self, num_poses, config: TransfuserConfig) -> None:
        super().__init__()
        self.agent_dropout = nn.Dropout(0.1)
        self.ego_dropout = nn.Dropout(0.1)
    ...
    def forward(
            ...
            ego_query, time_embed, global_img=None
        ):
        ...
        # ego cross attention
        traj_feature = traj_feature + self.ego_dropout(self.cross_ego_attention(traj_feature, ego_query,ego_query)[0])
...
class TrajectoryHead(nn.Module):
    def __init__(self, num_poses: int, config: TransfuserConfig):
        """
        ...
        self.diff_decoder = CustomTransformerDecoder(num_poses, config)
