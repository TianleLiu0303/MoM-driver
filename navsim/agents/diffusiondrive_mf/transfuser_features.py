...
class TransfuserFeatureBuilder(AbstractFeatureBuilder):
    ...
    #     """Inherited, see superclass for rwkv mixed agent cache."""
    #     features = {}
    #     num_history_frames = len(agent_input.cameras)

    #     camera_feature = [self._get_camera_feature(agent_input, index=i) for i in range(num_history_frames)]
    #     lidar_feature = [self._get_lidar_feature(agent_input, index=i) for i in range(num_history_frames)]
    ...
    #         ],
    #     )
    #     features["frames"] = torch.tensor(num_history_frames, dtype=torch.int32)
    ...
    def compute_features(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        ...
        camera_feature = [self._get_camera_feature(agent_input, index=i) for i in range(self._config.cam_seq_len)]
        lidar_feature = [self._get_lidar_feature(agent_input, index=i) for i in range(self._config.lidar_seq_len)]
