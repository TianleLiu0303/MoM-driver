...
from navsim.agents.diffusiondrive_mf.transfuser_config import TransfuserConfig
...
class TransfuserBackbone(nn.Module):
    ...
    def __init__(self, config: TransfuserConfig):
        ...
        if len(self.image_encoder.return_layers) > 4:
            start_index += 1
        self.kv_cache: Optional[List[Tuple[torch.Tensor, torch.Tensor]]] = []
    ...
    def forward(
        ...
        Args:
            image: Input image features tensor 
            lidar: Input LiDAR BEV features tensor
            use_cache: Whether to use feature caching
        Returns:
            Tuple of:
                features: Output BEV features tensor or None if not detect_boxes/use_bev_semantic
                fused_features: Fused image+lidar features tensor
                image_feature_grid: Image feature grid tensor or None if not use_semantic/use_depth
        """
        ...
        for i in range(4):
            ...
            image_features, lidar_features = self.fuse_features(image_features, lidar_features, i, use_cache)
    ...
    def fuse_features(self, image_features, lidar_features, layer_idx, use_cache=False):
        """
...
class GPT(nn.Module):
    ...
    def __init__(self, n_embd, config: TransfuserConfig, lidar_time_frames):
        super().__init__()
        self.n_embd = n_embd
        ...
        # time embedding parameter (learnable), sequence length
        # self.time_emb = nn.Parameter(torch.zeros(1, self.seq_len, self.n_embd))

        # whole embedding (learnable), (image + lidar) * seq_len
        self.emb = nn.Parameter(
            torch.zeros(
                1,
                self.seq_len * self.img_feature_size + lidar_time_frames * self.lidar_feature_size,
                self.n_embd
            )
        )
    ...
    def forward(
        ...
        # ---- 原有 token 拼接 / embedding 逻辑保持不变 ----
        x = self.drop(self.emb + token_embeddings)
        # presents: List[Tuple[torch.Tensor, torch.Tensor]] = []  # 用于缓存
        # if use_cache and (past_key_values is None or len(past_key_values) != len(self.blocks)):
        #     # 用 None 补齐到正确的层数
        #     past_key_values = [[None, None]] * len(self.blocks)
        #
        for i, block in enumerate(self.blocks):
            x = block(x, use_cache)
        # x = self.blocks(x, use_cache)
        x = self.ln_f(x)
...
class SelfAttention(nn.Module):
    ...
    # do not use cache when trainning
    def forward(self, x, past_k=None, past_v=None, use_cache=False):
        ...
        # concat with past key/value if present
...
class Block(nn.Module):
    ...
    def __init__(self, n_embd, n_head, block_exp, attn_pdrop, resid_pdrop):
        ...
        self.kv_cache: Optional[Tuple[torch.Tensor, torch.Tensor]] = [None, None]

    def clear_cache(self):
        self.kv_cache = [None, None]

    def forward(self, x: torch.Tensor,
                use_cache: bool = False) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]:
        if use_cache:
            attn_out, key, value = self.attn(self.ln1(x), past_k=self.kv_cache[0], past_v=self.kv_cache[1],
                                             use_cache=True)
        else:
            attn_out, key, value = self.attn(self.ln1(x))
