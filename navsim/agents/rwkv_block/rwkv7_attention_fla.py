...
class RWKV7Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        xr, xw, xk, xv, xa, xg = fused_addcmul_rwkv7(hidden_states, delta, self.x_r, self.x_w, self.x_k, self.x_v, self.x_a, self.x_g)
        ...
        if self.layer_idx == 0 and v_first is None:
            ...
            if v_first_seq_len < seq_len:
                ...
                v[:, kv_len:] = torch.lerp(v[:, kv_len:], v_first, self.v_lora(xv[:, kv_len:]).sigmoid())
            else:
...
class RWKV7_CrossAttention(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.x_q = nn.Parameter(torch.zeros(4, hidden_size))
        self.x_x = nn.Parameter(torch.zeros(2, hidden_size))
    ...
    def forward(
        ...
        **kwargs
        ...
        # TODO: consider cache 
        xr, xw, xa, xg = hidden_states.addcmul(xx_cross, self.x_q.view(4, 1, 1, -1)).unbind(0)
        xk, xv = hidden_states.addcmul(xx_keyval, self.x_x.view(2, 1, 1, -1)).unbind(0)
        
        r = self.r_proj(xr).to(torch.float)
        ...
        kk = k * self.k_k
        kk = l2_norm(kk.view(B, T, self.num_heads, -1)).view(B, T, C)
        k = k.addcmul(k * (a - 1), self.k_a)
