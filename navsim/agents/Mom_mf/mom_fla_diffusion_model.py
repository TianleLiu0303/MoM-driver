...
from navsim.agents.Mom_mf.mom_backbone import MoMBackbone
from navsim.agents.rwkv7_mf.rwkv_features import BoundingBox2DIndex
from navsim.agents.Mom_mf.mom_block_fla import Mom_DoubleSelfAttnBlock
...
class MLP(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, output_dim:int, bias=None) -> None:
        super().__init__()
        ...
        if bias is not None:
            bias_init = bias_init_with_prob(bias)
...
class MomModel(nn.Module):
    """Torch module for MoM."""

    def __init__(self, config: MomConfig):
        """
        Initializes MoM torch module.
        :param config: global config dataclass of MoM.
        """

        super().__init__()

        self._config = config
        self._backbone = MoMBackbone(config)
        ...
        self._query_embedding = nn.Embedding(config.num_bounding_boxes, config.rwkv_d_model)
        ...
        mom_decoder_config = copy.copy(config)
        mom_decoder_config.hidden_size = config.rwkv_d_model
        mom_decoder_config.num_heads = config.rwkv_d_num_head
        if mom_decoder_config.num_heads > 0:
            mom_decoder_config.head_dim = config.rwkv_d_model // mom_decoder_config.num_heads

        self._mom_decoder = nn.ModuleList(
            [
                Mom_DoubleSelfAttnBlock(
                    config=mom_decoder_config,
                    layer_idx=i,
                )
    ...
    def forward(
        ...
        status_feature: torch.Tensor = features["status_feature"][:,-1] # (B, _)
        ...
        # get the query
        agents_query = self._query_embedding.weight[None, ...].repeat(bs, 1, 1)
        for block in self._mom_decoder:
            agents_query, _, past_query, past_key_values, _, _ = block(
                query=agents_query,
                keyval=keyval,
        ...
        agents_query = self.ln_out(agents_query)

        # obtain the cross bev feature
        concat_cross_bev = bev_feature.permute(0, 2, 1).contiguous().view(-1, C, concat_cross_bev_shape[0], concat_cross_bev_shape[1])  # [bs * seq_len, C, H, W]
        ...
        cross_bev_feature = self.bev_proj(cross_bev_feature.flatten(-2, -1).permute(0, 2, 1))  # [bs * seq_len, T_expand, C]
        expanded_bs, T, C = cross_bev_feature.shape
        cross_bev_feature = cross_bev_feature.permute(0, 2, 1).contiguous().view(expanded_bs, C, bev_spatial_shape[0], bev_spatial_shape[1])

        # down sample the cross bev feature
        cross_bev_feature = F.interpolate(
            cross_bev_feature, 
            size=(self._config.bev_fuse_height, self._config.bev_fuse_width), 
            mode='bilinear', align_corners=False
        )
        cross_bev_feature = cross_bev_feature.flatten(-2, -1).permute(0, 2, 1).contiguous().view(bs, -1, C)

        # get the last frame of bev_feature_upscale 
        ...
        trajectory = self._trajectory_head(
            agents_query, cross_bev_feature, targets=targets, global_img=None
        )
...
class DiffMotionPlanningRefinementModule(nn.Module):
    def __init__(self, embed_dims=256, ego_fut_ts=8, if_zeroinit_reg=True, use_double_scorer=False):
        ...
        self.plan_score_branch = MLP(embed_dims + ego_fut_ts * 3, embed_dims, 6, bias=0.01)
        if use_double_scorer:
            self.plan_score_branch2 = MLP(embed_dims + ego_fut_ts * 3, embed_dims, 6, bias=0.5)
        ...
        self.agent_col_branch = MLP(embed_dims + ego_fut_ts * 3, embed_dims, 2 * 40 * 9, bias=0.01)
        self.area_branch = MLP(embed_dims + ego_fut_ts * 3, embed_dims, 40 * 2, bias=0.01)
...
class MoM_DiffusionDeCoderLayer(nn.Module):
    def __init__(self, idx, num_poses, config: MomConfig) -> None:
        super().__init__()
        self.config = config

        self.cross_bev_attention = Mom_DoubleSelfAttnBlock(
            config=config,
            layer_idx=idx * 2,
        )
        
        self.cross_agent_attention = Mom_DoubleSelfAttnBlock(
            config=config,
            layer_idx=idx * 2 + 1,
        )
    ...
    def forward(
            self, traj_feature, noisy_traj_points, bev_feature, 
            agents_query, time_embed, global_img=None
        ):
        # agent cross attention
        traj_feature, _, _, _, _, _ = self.cross_bev_attention(
            query=traj_feature,
            keyval=bev_feature,
        )
        ...
        # bev cross attention
        traj_feature, _, _, _, _, _ = self.cross_agent_attention(
            query=traj_feature,
            keyval=agents_query,
        )
        ...
        return poses_reg, poses_score, poses_score2, pred_agent_col, pred_area


class MoM_DiffusionDecoder(nn.Module):
    def __init__(self, num_poses, config: MomConfig) -> None:
        super().__init__()
        # torch._C._log_api_usage_once(f"torch.nn.modules.{self.__class__.__name__}")

        self.layers = nn.ModuleList(
            [
                MoM_DiffusionDeCoderLayer(
                    idx=i,
                    num_poses=num_poses,
                    config=config
    ...
    def forward(
        ...
        for layer in self.layers:
            poses_reg, poses_score, poses_score2, pred_agent_col, pred_area = layer(
                traj_feature, traj_points, bev_feature, 
                agents_query, time_embed, global_img
            )
...
class TrajectoryHead(nn.Module):
    def __init__(self, num_poses: int, config: MomConfig):
        """
        ...
        self.plan_anchor_encoder = MLP(512, self._d_model, self._d_model)
        ...
        self.diff_decoder = MoM_DiffusionDecoder(num_poses, mom_decoder_config)
    ...
    def forward(self, agents_query, bev_feature, targets=None, global_img=None) -> Dict[str, torch.Tensor]:
        """ Torch module forward pass """
        if self.training:
            return self.forward_train(agents_query, bev_feature, targets, global_img)
        else:
            return self.forward_test(agents_query, bev_feature, targets, global_img)
        
    def forward_train(self, agents_query, bev_feature, targets=None, global_img=None) -> Dict[str, torch.Tensor]:
        ...
        return {
            "trajectory": best_reg, "best_trajectory": trajectory_candidates, "denoised_trajectories": poses_reg_list[-1], 
            "score_logit": poses_score_list[-1], "score_logit2": poses_score_list2[-1] if poses_score_list2 else None, 
            "best_pdm_score": best_pdm_score, "average_pdm_score": average_pdm_score, 
            "pred_agent_col": pred_agent_col, "pred_area": pred_area
        }
    
    def forward_test(self, agents_query, bev_feature, targets=None, global_img=None) -> Dict[str, torch.Tensor]:
        ...
        return {"trajectory": best_reg, "denoised_trajectories": poses_reg, "score_logit": poses_score, "score_logit2": poses_score2, "best_pdm_score": best_pdm_score, "average_pdm_score": average_pdm_score, "pred_agent_col": pred_agent_col, "pred_area": pred_area}
...
if  __name__ == "__main__":
    ...
    output = model.forward(total_features, targets=None)
