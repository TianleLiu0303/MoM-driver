...
class RWKVModel(nn.Module):
    ...
    def __init__(self, config: RWKVConfig):
        ...
        self._keyval_len = config.lidar_seq_len * 8**2 + 1  # seq_len * 8x8 feature grid + status
        self._feature_len = 8**2
        self._bev_pos_embedding = nn.Parameter(torch.randn(1, self._feature_len, config.rwkv_d_model))
    ...
    def forward(
        ...
        bev_feature_upscale = bev_feature_upscale[torch.arange(bs), frames_indices]  # extract specific frame
        ...
        # add embedding to bev_feature
        expanded_bs, T, C = bev_feature.shape
        bev_feature = bev_feature + self._bev_pos_embedding
        bev_feature = bev_feature.contiguous().view(bs, -1, C) # [bs, seq_len * T, C]

        bev_padding = torch.zeros(bs, 1, C, device=bev_feature.device)
        ...
        bev_feature[torch.arange(bs), frames_feature * self._feature_len, :] = status_encoding
        keyval = bev_feature

        # get the specific frame cross bev feature
        B, _, C = keyval.shape
        concat_cross_bev = keyval[:, :-1, :].reshape(B, -1, self._feature_len, C)[torch.arange(B), frames_indices].squeeze(1)
