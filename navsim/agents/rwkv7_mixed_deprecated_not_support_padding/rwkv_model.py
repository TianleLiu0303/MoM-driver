...
from navsim.agents.rwkv7_mixed.rwkv_backbone import RWKVBackbone
...
class RWKVModel(nn.Module):
    ...
    def forward(self, features: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        #     bev_feature[i, frames_feature[i] * self._feature_len, :] = status_encoding[i, :]
        ...
        for block in self._rwkv_decoder:
            query_out, query_v_first, cross_query_v_first = block(
                query_out, keyval, query_v_first, cross_query_v_first, frames_feature
            )
