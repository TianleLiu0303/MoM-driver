...
class RWKVBackbone(nn.Module):
    ...
    def forward(self, image, lidar, past_features=[None, None, None, None], use_cache=False):
        """
        ...
        for i in range(4):
            ...
            image_features, lidar_features, past_features[i] = self.fuse_features(image_features, lidar_features, i, past_features[i], use_cache)
    ...
    def fuse_features(self, image_features, lidar_features, layer_idx, past_feature=None, use_cache=False):
        """
        ...
        image_features_layer, lidar_features_layer, past_feature = self.rwkvs[layer_idx](image_embd_layer, lidar_embd_layer, past_feature, use_cache)
...
class GPT_RWKV(nn.Module):
    ...
    def forward(
        ...
        # initial implementation with time embedding
        ...
        for block in self.blocks:
            x, _, past_key_values, v_first = block(x, v_first, past_key_values, use_cache)
        x = self.ln_f(x)   
