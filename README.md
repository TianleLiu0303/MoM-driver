...
## Cache data for multi frames
- bash scripts/training/run_load_cache_rwkv.sh
- You change the parameters in the *.sh* file
- *agent* must be *transfuser-based*
- set the value for *num_history_frames*


## Train rwkv_mf_agent (Multi Frame)
- current is *4* frames
- bash scripts/training/run_rwkv_mf_training.sh
- You change the parameters in the *.sh* file
- set the value for *num_history_frames* (multi frame)
- change the default value to adapt multi frame (such as training_cache_multi_*10*)
- change the sensor_config in the *-agent.py file : *SensorConfig.build_all_sensors(include=[0, 1, 2, 3])* to *SensorConfig.build_all_sensors(include=[i for i in range(4)])*
- change the *rwkv_config.py* :  *cam_seq_len: int = 4lidar_seq_len: int = 4*
