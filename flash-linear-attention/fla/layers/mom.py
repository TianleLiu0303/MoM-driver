...
class MomAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        # Build per-position masks for State-Frozen or its beta/g-only ablations.
        # We route the masks alongside hidden_states so every memory slot keeps
        # the correct per-token flags after MoM reordering.
        if frozen_from >= 0:
            freeze_beta_from = frozen_from
            freeze_g_from = frozen_from

        use_beta_mask = 0 <= freeze_beta_from < seq_len
        use_g_mask = 0 <= freeze_g_from < seq_len

        if use_beta_mask or use_g_mask:
            mask_features = []
            if use_beta_mask:
                _beta_mask = torch.ones(
                    batch_size,
                    seq_len,
                    1,
                ...
                _beta_mask[:, freeze_beta_from:, :] = 0.0
                mask_features.append(_beta_mask)
            if use_g_mask:
                _g_mask = torch.ones(
                    batch_size,
                    seq_len,
                ...
                _g_mask[:, freeze_g_from:, :] = 0.0
                mask_features.append(_g_mask)
            _hidden_aug = torch.cat([hidden_states] + mask_features, dim=-1)
        else:
            _hidden_aug = hidden_states

        _routed, indices, sorted_indices, max_len, mask, mask_2 = transform(
            _hidden_aug, routing_mask, self.num_memories, selected_memories, attention_mask)

        if use_beta_mask or use_g_mask:
            mask_dims = int(use_beta_mask) + int(use_g_mask)
            _mask_routed = _routed[..., -mask_dims:]
            hidden_states = _routed[..., :-mask_dims]
            mask_idx = 0
            if use_beta_mask:
                _beta_mask_routed = _mask_routed[..., mask_idx:mask_idx + 1]
                mask_idx += 1
            else:
                _beta_mask_routed = None
            if use_g_mask:
                _g_mask_routed = _mask_routed[..., mask_idx:mask_idx + 1]
            else:
                _g_mask_routed = None
        else:
            _beta_mask_routed = None
            _g_mask_routed = None
            hidden_states = _routed
        ...
        # Apply masks after projection.
        # beta * 0 = 0 -> no state write.
        # g    * 0 = 0 -> decay = exp(0) = 1 -> no attenuation.
        if _beta_mask_routed is not None:
            beta = beta * _beta_mask_routed
        if _g_mask_routed is not None:
            g = g * _g_mask_routed
